  ...
  group('BrashRepositoryTests', () {
    ...
    group('AddressInformation', () {
      test('Fetch existing AddressInformation', () async {
        const String url = 'http://213.168.213.236/bremereb/bify/bify.jsp?strasse=Emanuelstra%25DFe%26hausnummer=15';
        final AddressInformation response = await repository.getAddressInformation(url);
        print(response.currentStreet);
        expect(response.currentStreet.isNotEmpty, true);
      ...
      test('Fetch existing AddressInformation (street with umlaut)', () async {
        const String url = 'http://213.168.213.236/bremereb/bify/bify.jsp?strasse=%25D6tzener%2520Weg%26hausnummer=2';
        final AddressInformation response = await repository.getAddressInformation(url);
        print(response.currentStreet);
        expect(response.currentStreet.isNotEmpty, true);
        expect(response.allTrashEntries.isNotEmpty, true);
      ...
      test('Fetch non-existing AddressInformation', () async {
        const String url = 'http://213.168.213.236/bremereb/bify/bify.jsp?strasse=Emanuelstra%25DFe%26hausnummer=32';
    ...
    group('StreetPicker', () {
      ...
      test("Fetch all Streets starting with 'A'", () async {
        ...
        final Streets response = await repository.getStreets(query);
    ...
    group('HouseNumberPicker', () {
      ...
      test("Fetch Housenumbers for street 'Emanuelstraße'", () async {
        const String url = 'http://213.168.213.236/bremereb/bify/hausnummer.jsp?strasse=Emanuelstra%2525DFe';
        final HouseNumbers response = await repository.getHouseNumbers(url);
      ...
      test("Fetch HouseNumbers for a non-existing street 'Blubedi'", () async {
        const String url = 'http://213.168.213.236/bremereb/bify/hausnummer.jsp?strasse=Blubedi';
